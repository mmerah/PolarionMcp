...
async def get_project_info(project_id: str) -> str:
    ...
    Args:
        project_id: Project key (e.g., "MYPROJ")
...
async def search_workitems(
    ...
    """
    Search work items using Lucene query syntax.
    
    Args:
        project_id: Project key (e.g., "MYPROJ")
        query: Lucene query (e.g., "status:open AND type:defect")
    ...
    Query examples:
    - "type:requirement AND title:*OAuth*"
    - "assignee.id:john.doe AND NOT status:closed"
    """
    try:
        with PolarionDriver(
            url=settings.polarion_url,
...
@mcp.tool
async def discover_work_item_types(project_id: str, limit: int = 1000) -> str:
    """
    ...
    Note: Shows project-specific taxonomy (requirement, defect, task, etc.)
    """
    try:
        with PolarionDriver(
            url=settings.polarion_url,
